Feature: Collection access control
  ...
  Background:
    ...
    Given I have users
      ...
      | data_owner1@alveo.edu.au | dataOwner1 | One       |
      | data_owner2@alveo.edu.au | dataOwner2 | Two       |
      | researcher1@alveo.edu.au | Researcher | R         |
      | data_owner@alveo.edu.au  | Data       | Owner     |
    Given "data_owner@alveo.edu.au" has role "data owner"
    Given "data_owner1@alveo.edu.au" has role "data owner"
    Given "data_owner2@alveo.edu.au" has role "data owner"
    Given "researcher1@alveo.edu.au" has role "researcher"
    Given "researcher1@alveo.edu.au" has an api token
    ...
    Given Collections ownership is
      ...
      | austlit    | data_owner2@alveo.edu.au |
      | cooee      | data_owner1@alveo.edu.au |
    Given I ingest licences

  @javascript
  Scenario: Data Owner should be able to see all his collections
    Given I am logged in as "data_owner2@alveo.edu.au"
  ...
  Scenario: Data Owner should be able to see the details of his items
    Given I am logged in as "data_owner1@alveo.edu.au"
  ...
  Scenario: User should be able to see collections for which he has discover access
    Given "data_owner2@alveo.edu.au" has "discover" access to collection "cooee"
    Given I am logged in as "data_owner2@alveo.edu.au"
    Given I am on the home page
    Then I should see only the following collections displayed in the facet menu
      | collection |
  ...
  Scenario: User should see every item in a collection for which he has discover access
    Given "data_owner1@alveo.edu.au" has "discover" access to collection "austlit"
    Given I am logged in as "data_owner1@alveo.edu.au"
    Given I am on the home page
    And I have done a search with collection "austlit"
    And I should see "blacklight_results" table with
  ...
  Scenario: User should be able to see collections for which he has read access
    Given "data_owner2@alveo.edu.au" has "read" access to collection "cooee"
    Given I am logged in as "data_owner2@alveo.edu.au"
    Given I am on the home page
    Then I should see only the following collections displayed in the facet menu
      | collection |
  ...
  Scenario: User should see every item in a collection for which he has read access
    Given "data_owner1@alveo.edu.au" has "read" access to collection "austlit"
    Given I am logged in as "data_owner1@alveo.edu.au"
    Given I am on the home page
    And I have done a search with collection "austlit"
    And I should see "blacklight_results" table with
...
#----------- No Access
  ...
  Scenario: Download document which exists but I have not access
    When I make a JSON request for the document content page for file "1-001-plain.txt" for item "cooee:1-001" with the API token for "researcher1@alveo.edu.au"
    Then I should get a 403 response code
...
#----------- Discover Access
  Scenario: Get item details for an item I have discover access
    Given "researcher1@alveo.edu.au" has "discover" access to collection "cooee"
    When I make a JSON request for the catalog page for "cooee:1-001" with the API token for "researcher1@alveo.edu.au"
    Then I should get a 403 response code
